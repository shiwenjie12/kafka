...
object CoreUtils extends Logging {
  ...
  /**
    * 将给定的方法包装成 a java.lang.Runnable
    *
  ...
  def registerMBean(mbean: Object, name: String): Boolean = {
    try {
      ...
      mbs synchronized {
        ...
        mbs.registerMBean(mbean, objName) // 注册
        true
      }
    } catch {
      case e: Exception => {
  ...
  /**
    * 创建具有给定类名的类的实例  （: _* ）可变参数
    */
  def createObject[T <: AnyRef](className: String, args: AnyRef*): T = {
    val klass = Class.forName(className, true, Utils.getContextOrKafkaClassLoader()).asInstanceOf[Class[T]]
    val constructor = klass.getConstructor(args.map(_.getClass): _*)// 获取传入参数格式的构造函数
    constructor.newInstance(args: _*)// 实例化
  }
