...
class KafkaController(val config: KafkaConfig, zkClient: KafkaZkClient, time: Time, metrics: Metrics, initialBrokerInfo: BrokerInfo,
  ...
  val controllerContext = new ControllerContext // 控制器上下文
  ...
  /**
   * 该回调由动物园领导者选举人在选择当前经纪人作为新的控制者时调用。
   * 它在成为控制器状态变化时做了以下事情 -
   * 1.注册控制器时期改变的监听器
   * 2.递增控制器时期
   * 3.初始化控制器的上下文对象，该对象保存当前主题的缓存对象，所有现有分区的实时代理和领导。
   * 4.启动控制器的频道管理器
   * 5.启动副本状态机
   * 6.启动分区状态机
   * 如果在成为控制器时遇到任何意外的异常/错误，它将作为当前控制器退出。
   * 这确保了另一个控制器的选举将被触发，并且总会有一个主动服务的控制器
   */
  private def onControllerFailover() {
    ...
    // 在从zookeeper读取真实来源之前，注册侦听器以获得代理/主题回调。
    // 子节点改变
    ...
    initializeControllerContext()  // 初始化上下文
    ...
    sendUpdateMetadataRequest(controllerContext.liveOrShuttingDownBrokerIds.toSeq) // 发送更新元数据请求
    ...
    topicDeletionManager.tryTopicDeletion()  // 尝试删除主题
    ...
    if (config.tokenAuthEnabled) { // 进行token过期处理
  ...
  /**
    * 当当前broker辞去控制人时，该调用由zookeeper领队选举人调用。 这是清理内部控制器数据结构所必需的
   */
  ...
  private def registerBrokerModificationsHandler(brokerIds: Iterable[Int]): Unit = {
    ...
    brokerIds.foreach { brokerId =>
      ...
      zkClient.registerZNodeChangeHandler(brokerModificationsHandler) // 注册broker修改器
  ...
  /**
   * 该回调由主题变更回调调用，失败代理列表作为输入。
   * 它执行以下操作 -
   * 1.将新创建的分区移动到NewPartition状态
  ...
  /**
   * 此回调由重新分配的分区侦听器调用。 当管理员命令启动分区重新分配时，它会创建触发动物园管理员侦听器的/ admin / reassign_partitions路径。
   * 通过代码中列出的几个步骤重新分配分区的副本。
   *
  ...
  private def onPartitionReassignment(topicPartition: TopicPartition, reassignedPartitionContext: ReassignedPartitionsContext) {
    ...
    if (!areReplicasInIsr(topicPartition, reassignedReplicas)) { // 和ISR有出入
      ...
      //2. 将LeaderAndIsr请求发送到OAR + RAR中的每个副本（AR作为OAR + RAR）。
  ...
  private def maybeTriggerPartitionReassignment(topicPartitions: Set[TopicPartition]) {
    topicPartitions.foreach { tp =>
      if (topicDeletionManager.isTopicQueuedUpForDeletion(tp.topic)) { // 如果删除队列中包含主题，则跳过
        ...
        val newReplicas = reassignedPartitionContext.newReplicas  // 新的分区
        val topic = tp.topic
        controllerContext.partitionReplicaAssignment.get(tp) match {
          case Some(assignedReplicas) =>  // 已分配的分区
            if (assignedReplicas == newReplicas) { // 已经分配
  ...
  private def initializeControllerContext() {
    ...
    controllerContext.liveBrokers = zkClient.getAllBrokersInCluster.toSet // 存活的broker
    controllerContext.allTopics = zkClient.getAllTopicsInCluster.toSet // 所有的主题
    // 注册分区修改器
    ...
    startChannelManager()
    initializePartitionReassignment()  // 初始化重新分配分区
  ...
  private def fetchTopicDeletionsInProgress(): (Set[String], Set[String]) = {
    ...
    val topicsIneligibleForDeletion = topicsWithOfflineReplicas | topicsForWhichPartitionReassignmentIsInProgress // 取交集
    info(s"List of topics to be deleted: ${topicsToBeDeleted.mkString(",")}")  // 要删除的主题列表
    info(s"List of topics ineligible for deletion: ${topicsIneligibleForDeletion.mkString(",")}")  // 不适合删除的主题列表
  ...
  private def readControllerEpochFromZooKeeper() {
    ...
    epochAndStatOpt.foreach { case (epoch, stat) => // 设置控制器上下文
  ...
  /**
   * 将选定分区的领导者信息发送给选定的代理，以便他们可以正确响应元数据请求
   *
  ...
  case class TopicDeletionStopReplicaResponseReceived(stopReplicaResponseObj: AbstractResponse, replicaId: Int) extends ControllerEvent {
    ...
    override def process(): Unit = {
      ...
      // 将所有失败的副本移动到ReplicaDeletionIneligible
  ...
  private def elect(): Unit = {
     ...
     */
    if (activeControllerId != -1) { // 如果已经创建控制器节点，则直接返回
  ...
  case class PartitionModifications(topic: String) extends ControllerEvent {
    ...
    override def process(): Unit = {
      ...
      val partitionsToBeAdded = partitionReplicaAssignment.filter(p =>  // 增加的分区
      ...
      if (topicDeletionManager.isTopicQueuedUpForDeletion(topic))
        ...
        if (partitionsToBeAdded.nonEmpty) {
          ...
          controllerContext.partitionReplicaAssignment ++= partitionsToBeAdded // 更新上下文中已分配的分区
  ...
  case object PartitionReassignment extends ControllerEvent {
    ...
    override def process(): Unit = {
      ...
      // 如果路径不存在以便检测未来的重新分配，我们需要注册观察者，并且我们获得免费的“路径存在”检查
      if (zkClient.registerZNodeChangeHandlerAndCheckExistence(partitionReassignmentHandler)) {
        ...
        // 在调用`maybeTriggerPartitionReassignment`之前填充`partitionsBeingReassigned`并重新分配所有分区（请参阅方法文档以了解原因）
  ...
  case object IsrChangeNotification extends ControllerEvent {
    ...
    override def process(): Unit = {
      if (!isActive) return
      val sequenceNumbers = zkClient.getAllIsrChangeNotifications // 更改的序号
      try {// 从zk获取的变化的isr的tp
        val partitions = zkClient.getPartitionsFromIsrChangeNotifications(sequenceNumbers)
        if (partitions.nonEmpty) {
          updateLeaderAndIsrCache(partitions) // 更新领导者中的ISR缓存
          processUpdateNotifications(partitions) // 处理更新唤醒，要通知到其他broker
        }
      } finally {
        // 删除唤醒节点
  ...
  case object ControllerChange extends ControllerEvent {
    ...
    override def process(): Unit = {
      val wasActiveBeforeChange = isActive // 在改变之前，本机的控制器是否是存活的
      zkClient.registerZNodeChangeHandlerAndCheckExistence(controllerChangeHandler)
      activeControllerId = zkClient.getControllerId.getOrElse(-1)
      if (wasActiveBeforeChange && !isActive) { // 如果之前是，但是现在不是控制器，则注销控制器的相关操作
        onControllerResignation()
      }
    }
  }
  ...
  // 由于countDownLatch字段，我们必须等待控制的过期注销
  class Expire extends ControllerEvent {
    ...
    override def process(): Unit = {
      ...
      activeControllerId = -1
      onControllerResignation() // 控制器注销处理
    }
